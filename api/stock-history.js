...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (!symbol) {
      return res.status(400).json({ error: 'Stock symbol required. Usage: ?symbol=AAPL&range=1M' });
    }
    ...
    switch (range) {
      case '1M':
        ...
        break;
      case '3M':
        from = to - (90 * 24 * 60 * 60); // 90 days ago
        break;
      case '6M':
        from = to - (180 * 24 * 60 * 60); // 180 days ago
        break;
      case '1Y':
        from = to - (365 * 24 * 60 * 60); // 365 days ago
        break;
      case '5Y':
        from = to - (5 * 365 * 24 * 60 * 60); // 5 years ago
        break;
      default:
        from = to - (365 * 24 * 60 * 60); // Default to 1 year
    }
